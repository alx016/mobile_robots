...
class PathFollower:
    ...
    def calculate_linear_velocity(self, error):
        ...
        velocity_command.linear.x = v_max_linear * np.tanh(velocity_command.linear.x / v_max_linear)
        velocity_command.linear.y = v_max_linear * np.tanh(velocity_command.linear.y / v_max_linear)
