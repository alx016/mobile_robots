...
class PathPlanner:
    ...
    def is_valid(self, node, map_array):
        x, y = node
        return 0 <= x < map_array.shape[1] and 0 <= y < map_array.shape[0] and map_array[y, x] < 10

    # def heuristic(self, a, b):
    #     # Heuristica para A* (distancia Manhattan)
    #     return abs(a[0] - b[0]) + abs(a[1] - b[1])
    def heuristic(self, goal, node, map_array):
        penalty_factor = 10

        distance =  np.sqrt((goal[0] - node[0]) ** 2 + (goal[1] - node[1]) ** 2)

        penalty = penalty_factor * (10 - map_array[node[1], node[0]])
