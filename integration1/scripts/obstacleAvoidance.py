...
class obstacleAvoidance:
    def __init__(self):
        ...
        self.a_vel  =    rospy.get_param("a_vel/val", default = 0.15)              #rotational velocity
        self.l_vel  =    rospy.get_param("l_vel/val", default = 0.12)              #rotational velocity
    ...
    def lidar_callback(self, data):
        ...
        if (self.key == True):
            if right_distance < self.min_dist:
                self.turn_left()
            elif right_distance > self.min_dist + 0.01:
                self.turn_right()
            else:
                # y_vel = -self.kp*(right_distance-0.25)
                # if (y_vel > 0.15):
                #     y_vel = 0.15
                # if (y_vel < -0.15):
                #     y_vel = -0.15
                self.move_forward(forward_distance, 0)
        else:
...
if __name__ == '__main__':
    ...
    node_handler = obstacleAvoidance()
    rospy.spin()